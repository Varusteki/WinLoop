...
### 5.15 尺寸单位：DIP 基准 + 本屏 DPI 自适应

**问题**：菜单的所有尺寸配置都是固定像素。在 200% 缩放的 4K 屏上，
同一个「半径 90」只占屏幕的一小格 —— 视觉大小只剩 1080p 下的四分之一（按面积算），
用户在 1080p 上调舒服的值换台机器就不可用。

**方案**：尺寸字段的语义定为「**100% 缩放下的逻辑像素（DIP 基准）**」，
菜单渲染时乘以「本屏 DPI / 96」。

| 缩放 | 系数 | 效果 |
|---|---|---|
| 100% | ×1.00 | 与老版本**像素级一致** |
| 125% | ×1.25 | 菜单物理尺寸 +25%，视觉大小不变 |
| 150% | ×1.50 | 同上 |
| 200%（4K 常见） | ×2.00 | 菜单物理尺寸翻倍，视觉大小与 100% 屏相同 |

**只追随 DPI，不追随分辨率。** DPI 缩放表达的是操作系统的用户偏好
（"我要多大的界面元素"）；分辨率本身不改变这个偏好 ——
4K + 100% 缩放的用户就是想要小元素、大工作区，强行按分辨率放大等于夺走他的选择。

**实现落点**：

- `Models/SizingScale.cs`（新增）：`FromVisual(Visual)` 用
  `PresentationSource.FromVisual(...).CompositionTarget.TransformToDevice.M11`
  取**该元素所在显示器**的缩放系数（Per-Monitor V2 下正是"本屏"）；
  无 PresentationSource 或系数非法一律退回 `1.0`。
- `Models/AppConfig.cs`：新增运行时字段 `SizingScale`（`[JsonIgnore]`，**不落盘**——
  它是运行时状态，写进 config.json 会在换屏后留下脏值）。
- `Menus/RadialMenu.cs`：新增 `protected double Scaled(double baseValue)`，
  **子类读任何半径/粗细都必须经过它**。这是唯一的换算入口。
- `UI/MenuOverlayWindow.xaml.cs`：`ShowAt` 里在 `Show()` 之后、`CreateMenu` 之前
  设 `_config.SizingScale = SizingScale.FromVisual(this)`（顺序不能颠倒：
  `Show()` 才有 PresentationSource，而菜单在 `Initialize` 里读这个系数）。
- `UI/SettingsWindow.xaml.cs`：两处预览固定传 `SizingScale.PreviewScale`（= 1.0）。
  预览的职责是表达"我调大了一点"这种**相对**变化，跟着 DPI 走反而会在
  不同机器上显示不同大小，看不出自己改了什么。

**配置兼容（v1 → v2）**：

老 `config.json` 里没有单位标记，反序列化后 `SizingUnitVersion = 0`（= v1）。
v1 的"固定像素"与 v2 的"DIP 基准"在 100% 缩放时**数值完全等价**，
...
**回归**：`CheckDpiScaling` 覆盖 4 档缩放（1.0 / 1.25 / 1.5 / 2.0）× 4 种样式，验证
① `VisualRadius` 严格成比例、② 死区同比例、③ 方向判定逐点不受缩放影响（72 点/档）、
④ 缩放系数为 0 / 负数 / NaN 时安全退回 1.0 而非画出 NaN 图形。
